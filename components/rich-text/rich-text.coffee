...
export default
	...
	methods:
		...
		renderNodes: ->
			...
			[BLOCKS.TABLE]: (node, key, create, next) => @wrapElement('table', node, key, create, next)
		...
		# Render function that wraps element on a div
		wrapElement: (element, node, key, create, next) ->
			create 'div', {}, create(element, { key }, next(node.content, key, create, next))
